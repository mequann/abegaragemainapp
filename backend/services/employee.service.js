...
const createEmployee = async (employee) => {
  ...
  try {
    ...
    const employeeInfoResult = await conn.executeQuery(employeeInfoQuery, [
      employee_id,
      employee.employee_first_name,
      employee.employee_last_name,
      employee.employee_phone,
    ]);
    const employeePassResult = await conn.executeQuery(employeePassQuery, [
      employee_id,
      hashedPassword,
    ]);
    const employeeRoleResult = await conn.executeQuery(employeeRoleQuery, [
      employee_id,
...
const getEmployees = async () => {
  //write query with inner jion  with all tables to get all the employees
  const query = `
        SELECT * 
        FROM employee
        INNER JOIN employee_info ON employee.employee_id = employee_info.employee_id
